...
- **Per-flavour ingredient structure:** `Product.ingredients_by_flavour: dict[str, list[str]] | None` preserves the per-flavour breakdown for multi-flavour products. The flat `Product.ingredients` field is kept unchanged for backward compat (eval/consistency/draft tools use it). `check_field` enriches ingredient answers with `per_flavour` and `all_flavours` when structured data is available (see G-31).
- **Full-page descriptive text:** `Product.page_text: str | None` holds concatenated prose from the product page — benefit bullets from `<div class="old-description">` (parsed from the RSC Flight decoded text, not the raw DOM), `why_its_healf`, and `suggested_use` metafields. RSC pointer values (e.g. `"$24"`) are filtered. `load_full_product` populates it; `app.py` injects it into the per-turn agent context as a `--- Page description text ---` block (see G-32). No new dependencies — uses existing `selectolax` + `extract_rsc_flight`.
- **Shopify JSON endpoints blocked (G-33):** All `/products/{handle}.json` and `/products.json` probes return HTML — Healf's Next.js App Router intercepts them. RSC flight payload is the only programmatic data source.
- **RSC pointer hex regex (G-34):** `_RSC_PTR_RE = re.compile(r"^\$[0-9a-f]+$", re.IGNORECASE)` — hex pointers like `$1e` are matched; previous pattern `^\$\d+$` only caught decimal-digit pointers.
- **Multi-source image merge (G-35):** `Product.images` is now populated from: (1) JSON-LD `image` scalar, (2) `variant_base_images` metafield parsed as JSON (before `_clean_metafield_text`), (3) CDN regex fallback on the full RSC flight. `load_full_product` merges with order-preserving dedup.
- **Claims sourced from `old-description` `<li>` items (G-34/Wave B):** `_claims_from_old_description(html)` extracts `<li>` bullets from `<div class="old-description">` in the RSC flight. Takes priority over metafield fallback; metafield fallback is also pointer-filtered.
- **Vision OCR pass (G-36):** `score_images` returns `aggregated_on_pack_text` (Gemini OCR of label) and `any_nutrition_panel` (bool). `SYSTEM_PROMPT` directs the agent to use these fields for nutrition/serving-size questions. Advisory only — ~30% extra output tokens per call.
- **`run_agent_turn` 3-tuple + Streamlit debug (G-37):** Returns `(text, trace, debug)`. `debug` captures system prompt, injected product context, and per-iteration LLM messages/response/usage/latency. Two new Streamlit expanders: "debug — prompts & messages" and "debug — usage & timing". In-memory only (no disk persistence yet).
